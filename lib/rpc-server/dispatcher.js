...
var logger = require('pomelo-logger').getLogger('rpc-log', __filename);
...
pro.route = function(tracer, msg, cb) {
  ...
  if (method.constructor.name === 'AsyncFunction') {
    method.apply(service, args).then(r => {
      cb(null, r);
    }).catch(e => {
      logger.error('call [%s.%s.%s] error', msg.serverType, msg.service, msg.method)
