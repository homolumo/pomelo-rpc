...
var doFilter = function(tracer, err, serverId, msg, opts, filters, index, operate, cb) {
  ...
  var fun = function(target, message, options) {
    index++;
  ...
  var action;
  if (typeof filter === 'function') action = filter;
  else if (typeof filter[operate] === 'function') action = filter[operate];

  if (action) {
    if (action.constructor.name === 'AsyncFunction') {
      action(serverId, msg, opts).then((r) => {
        if (r) fun(r.target, r.message, r.options);
        else fun();
      }).catch(e => doFilter(tracer, target, serverId, msg, opts, filters, index, operate, cb));
    } else {
      action(serverId, msg, opts, fun);
    }
    return;
  } else {
    index++;
