...
var genObjectProxy = function(serviceName, origin, attach, proxyCB) {
  ...
  if ((typeof origin.__clazz) === 'function') {
    for (const field of Object.getOwnPropertyNames(origin.constructor.prototype)) {
      if (field !== 'constructor') {
        res[field] = genAsyncFunctionProxy(serviceName, field, origin, attach, proxyCB);
      }
    }
  } else {
...
var genAsyncFunctionProxy = function(serviceName, methodName, origin, attach, proxyCB) {
  return (function() {
    var proxy = async function() {
      ...
      // var args = Array.prototype.slice.call(arguments, 0);
      await proxyCB(serviceName, methodName, args, attach);
    };

    proxy.toServerAsync = async function() {
      // var args = arguments;
      var len = arguments.length;
      var args = new Array(len);
      for (var i = 0; i < len; i++) {
      ...
      await proxyCB(serviceName, methodName, args, attach, true);
    };
